...
describe("GoFlow", function () {
  ...
  describe("TransferFrom", () => {
    it("Should approve a spender to be able to transfer owner's tokens", async () => {
      ...
      await approve.wait();
      const transferFrom = await goflow
        .connect(otherAccount)
        .transferFrom(owner.address, otherAccount.address, 20);
    ...
    it("Should not allow spender to transfer more tokens than they have", async () => {
      ...
      expect(await goflow.approve(otherAccount.address, 20)).to.be.revertedWith(
